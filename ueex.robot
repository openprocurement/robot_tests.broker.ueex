...
Підготувати клієнт для користувача
  ...
  Run Keyword If  '${username}' != 'ueex_Viewer'  Login  ${username}
...
Завантажити документ
  ...
  ueex.Пошук тендера по ідентифікатору  ${username}  ${TENDER}
...
Завантажити документ в тендер з типом
  ...
  ueex.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
  ueex.Завантажити документ до форми з типом  ${username}  ${tender_uaid}  ${filepath}  illustration
...
Завантажити ілюстрацію
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  ueex.Завантажити документ в тендер з типом  ${username}  ${tender_uaid}  ${filepath}  illustration
...
Додати публічний паспорт активу
  [Arguments]  ${username}  ${tender_uaid}  ${vdr_url}
  ueex.Завантажити документ в тендер з типом  ${username}  ${tender_uaid}  ${filepath}  x_dgfPublicAssetCertificate

Додати офлайн документ
  [Arguments]  ${username}  ${tender_uaid}  ${accessDetails}
  ueex.Завантажити документ в тендер з типом  ${username}  ${tender_uaid}  ${filepath}  x_dgfAssetFamiliarization
...
Отримати документ
  ...
  Run Keyword If  ${SEARCH_MODE}=='assets'  ueex.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
  ...  ELSE IF  ${SEARCH_MODE}=='lots'  ueex.Пошук лоту по ідентифікатору  ${username}  ${tender_uaid}
  ...  ELSE  ueex.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Дискваліфікувати постачальника
  ...
  ueex.Оновити сторінку з тендером  ${username}  ${tender_uaid}
  ${award_index}=  inc  ${award_index}
...
Отримати інформацію із об'єкта МП
  ...
  ${return_value}=  Run Keyword  ueex.Отримати інформацію про ${fieldname}
  [return]  ${return_value}

Отримати інформацію з активу об'єкта МП
  [Arguments]  ${username}  ${asset_uaid}  ${item_id}  ${field_name}
  ${return_value}=   Run KeyWord   ueex.Отримати інформацію з поля предмету  ${username}  ${asset_uaid}  ${item_id}  ${field_name}
  [Return]  ${return_value}
...
Внести зміни в об'єкт МП
  ...
  Run KeyWord  ueex.Внести зміни в об'єкт МП поле  ${fieldname}  ${fieldvalue}
  Click Element  id=btnPublic

Внести зміни в об'єкт МП поле 
  [Arguments]  ${fieldname}  ${fieldvalue}
  ueex.Внести зміни до елементу за шляхом  //*[@name = '${fieldname}']  ${fieldname}  ${fieldvalue}

Внести зміни в актив об'єкта МП
  [Arguments]  ${username}  ${item_id}  ${asset_uaid}  ${fieldname}  ${fieldvalue}
  ueex.Пошук об’єкта МП по ідентифікатору  ${username}  ${asset_uaid}
  Run KeyWord  ueex.Внести зміни до елементу за шляхом  //div[starts-with(@id, 'pn_w_item') and contains(@class, '${item_id}')]//*[@name= '${fieldname}']  ${fieldname}  ${fieldvalue}
  Click Element  id=btnPublic
...
Завантажити документ в об'єкт МП з типом
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}  ${doc_type}
  ueex.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
  ueex.Завантажити документ до форми з типом  ${username}  ${tender_uaid}  ${filepath}  ${doc_type}

Завантажити ілюстрацію в об'єкт МП
  [Arguments]  ${username}  ${asset_uaid}  ${filepath}
  ueex.Завантажити документ в об'єкт МП з типом  ${username}  ${asset_uaid}  ${filepath}  illustration

Завантажити документ для видалення об'єкта МП
  [Arguments]  ${username}  ${asset_uaid}  ${filepath}
  ueex.Завантажити документ в об'єкт МП з типом  ${username}  ${asset_uaid}  ${filepath}  cancellationDetails
...
Додати умови проведення аукціону номер 0
  ...
  ueex.Пошук лоту по ідентифікатору  ${username}  ${lot_uaid}
...
Оновити сторінку з об'єктом МП
  [Arguments]  ${username}  ${lot_uaid}
  ueex.Пошук об’єкта МП по ідентифікатору  ${username}  ${lot_uaid}
...
Отримати інформацію з активу лоту
  ...
  ${return_value}=   Run KeyWord   ueex.Отримати інформацію з поля предмету  ${username}  ${lot_uaid}  ${item_id}  ${field_name}
  [Return]  ${return_value}

Завантажити документ в лот з типом
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}  ${doc_type}
  ueex.Пошук лоту по ідентифікатору  ${username}  ${tender_uaid}
  ueex.Завантажити документ до форми з типом  ${username}  ${tender_uaid}  ${filepath}  ${doc_type}
...
Завантажити ілюстрацію в лот
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  ueex.Завантажити документ в лот з типом  ${username}  ${tender_uaid}  ${filepath}  illustration

Завантажити документ для видалення лоту
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  ueex.Завантажити документ в лот з типом  ${username}  ${tender_uaid}  ${filepath}  cancellationDetails
...
Внести зміни в лот
  [Arguments]  ${username}  ${tender_uaid}  ${fieldname}  ${fieldvalue}
  ueex.Пошук лоту по ідентифікатору  ${username}  ${tender_uaid}
  Run KeyWord  ueex.Внести зміни в лот поле  ${fieldname}  ${fieldvalue}
  Click Element  id=btnPublic
...
Внести зміни в умови проведення аукціону
  ...
  ueex.Пошук лоту по ідентифікатору  ${username}  ${tender_uaid}
  Run Keyword If  '${fieldname}' == 'auctionPeriod.startDate'  ueex.Внести дату та час  _auction_${item_id}_auctionPeriod_startDate  ${fieldvalue}
